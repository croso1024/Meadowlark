/*
    ...
    -1. 連接database
        ...
        我們使用的ODM是mongoose , 
        p.s 所連接的db(或是在沒有該db時自動創建)的名稱是在connectURL內設定,或著給connect()方法中提供有dbName屬性的物件
...
mongoose.connect(connectURL , {dbName:"Meadowlark"}) ; 
...
// Ch18 , 在進行身份驗證前,加入使用者資料的資料模型,保存&存取使用者
// 除了透過mongodb自己本身maintain的key來存取外 , 我們有自己的authId屬性,
// 由於目標是要整合可能多個第三方身份驗證服務 , 因此我們的authId要例如"fb:xxxxx" , "google:yyyyy" ,以避免ID衝突
...
module.exports = {
    ...
    addUser : async data => {
        ...
        let d = new User(data).save()
        console.log("Check moogose save() return value:")
        console.log(d) ; 
        return d ;

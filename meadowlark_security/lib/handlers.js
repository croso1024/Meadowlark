...
// ch18 , 在home面加入user資訊 , 另外我自己再額外加入了role的資訊,因為handlerbars似乎不能直接判斷屬性值,
// 而只能用屬性的存在和bool值的方式去進行選擇性渲染, 因此這邊我比較土砲的方式做了role equal to employee的判斷
exports.home = (req , res) => res.render('home' , 
    {username : req.user && req.user.name , employee : ( req.user &&  req.user.role === "employee")? true:false}
    ) ; 
